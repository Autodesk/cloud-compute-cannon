...
class BatchComputeDockerTurbo
{
	...
	public static function executeTurboJobV2(redis :RedisClient, job :BatchProcessRequestTurboV2, docker :Docker, machineId :MachineId, log :AbstractLogger) :Promise<JobResultsTurboV2>
	{
		...
		var ensureImage = BatchComputeDocker.ensureDockerImage.bind(docker, job.image, log, job.imagePullOptions);
		var runContainer = runContainerTurboV2.bind(job, docker, log);
		var getOutputs = getOutputsTurboV2.bind(job, _, log);
		...
		return Promise.promise(true)
			...
			.then(function(_) {
				var jobResultsFinal :JobResultsTurboV2 = {
					id: jobId,
					outputs: outputFiles,
					error: error,
					stdout: resultBlob != null ? resultBlob.stdout : [],
	...
	static function createTarStreamFromFilesV2(files :Array<ComputeInputSource>) :TarPack
	{
		var tarStream = TarStream.pack();
		for (file in files) {
			tarStream.entry({name: file.name}, new Buffer(file.value, file.encoding));
		}
		tarStream.finalize();
		return tarStream;
	}
	...
	static function runContainerTurboV2(job :BatchProcessRequestTurboV2, docker :Docker, log :AbstractLogger) :Promise<RunContainerResultTurbo>
	{
		var jobId = job.id;
		...
		return Promise.promise(true)
			.pipe(function(_) {
				...
				var inputStream = modifiedPathInputs != null ? createTarStreamFromFilesV2(modifiedPathInputs) : null;
	...
	{
		var jobId = job.id;
		//Turbo jobs are never retried, so keep the attempt to 1
		var outputVolumeName = JobTools.getWorkerVolumeNameOutputs(jobId, 1);
	...
	static function getOutputsTurboV2(job :BatchProcessRequestTurboV2, container :DockerContainer, log :AbstractLogger) :Promise<Array<ComputeInputSource>>
	{
		...
		return cast DockerDataTools.getDataFilesFromContainerV2(container, path)
			.errorPipe(function(err) {
				log.warn({error:err, f:'getOutputsTurboV2'});
				return Promise.promise(null);
			});
	}
}