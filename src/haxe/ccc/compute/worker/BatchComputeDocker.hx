...
class BatchComputeDocker
{
	 ...
	 * @return           [description]
	 */
	public static function executeJob(redis :RedisClient, job :DockerBatchComputeJob, attempt :Int, dockerOpts:DockerConnectionOpts, fs :ServiceStorage, killContainer :Promise<Bool>, log :AbstractLogger) :ExecuteJobResult
	{
		...
		var parentLog = log;
		log = parentLog.child({jobId:jobId, attempt:attempt, step:'executing_job'});
		...
		var inputVolumeName = job.inputs.length > 0 ? JobTools.getWorkerVolumeNameInputs(jobId, attempt) : null;
		...
		var copyInputs = copyInputsInternal.bind(job, attempt, dockerOpts, fs, redis, log);
		var copyOrCreateImage = copyOrCreateImageInternal.bind(job, attempt, docker, redis, log);
		var createOutputVolume = function() return DockerDataTools.createVolume(outputsVolume).thenTrue();
		var runContainer = runContainerInternal.bind(job, attempt, docker, redis, killContainer, log);
		var copyOutputs = copyOutputsInternal.bind(job, attempt, dockerOpts, fs, redis, log);
		var copyLogs = copyLogsInternal.bind(jobId, attempt, docker, resultsStorageRemote, redis, log);
		...
		function setStatus(status :JobWorkingStatus) {
			...
			return JobStateTools.setWorkingStatus(jobId, attempt, status);
		}

		var p = Promise.promise(true)
			// .pipe(function(_) {
			// 	return removePossibleExistingVolumesIfSecondJobAttempt(docker, jobId, inputVolumeName, outputVolumeName);
			// })
			.pipe(function(_) {
				return JobStateTools.getWorkingStatus(jobId, attempt)
			...
			.pipe(function(_) {
				if (jobWorkingStatus == JobWorkingStatus.CopyingInputsAndImage) {
					return Promise.whenAll([
						...
						.pipe(function(_) {
							return setStatus(JobWorkingStatus.ContainerStarting);
						});
				} else {
					return Promise.promise(true);
				}
			...
			.pipe(function(_) {
				if (jobWorkingStatus == JobWorkingStatus.ContainerStarting || jobWorkingStatus == JobWorkingStatus.ContainerRunning) {
					return runContainer()
						.pipe(function(resultsBlob) {
							log.debug(resultsBlob);
						...
						return Promise.whenAll(
							[
								inputVolumeName == null ? Promise.promise(true) : DockerPromises.removeVolume(docker.getVolume(inputVolumeName))
									...
									.errorPipe(function(err) {
										log.warn({CleanupStep: CleanupStep.CleanupStep_03_Remove_Input_Volume, error:Json.stringify(err), success:false});
										return Promise.promise(false);
									}),
								DockerPromises.removeVolume(docker.getVolume(outputVolumeName))
									...
									.errorPipe(function(err) {
										log.warn({CleanupStep: CleanupStep.CleanupStep_04_Remove_Output_Volume, volume:outputVolumeName, error:Json.stringify(err), success:false});
										return Promise.promise(false);
									})
							])
							.thenTrue()
	...
	/**
	 * If a job is retried and the same worker picks it up
	 * it might still have volumes from the previous attempt.
	 * This can cause problems, so remove the existing volumes
	 * before starting a new job.
	 * @param  docker           :Docker       [description]
	 * @param  inputVolumeName  :String       [description]
	 * @param  outputVolumeName :String       [description]
	 * @return                  [description]
	 */
	// static function removePossibleExistingVolumesIfSecondJobAttempt(docker :Docker, jobId :JobId, inputVolumeName :String, outputVolumeName :String) :Promise<Bool>
	// {
	// 	return Promise.promise(true);
	// 	return JobStatsTools.getAttempts(jobId)
	...
	static function copyInputsInternal(job :DockerBatchComputeJob, attempt :Int, dockerOpts :DockerConnectionOpts, fs :ServiceStorage, redis :RedisClient, log :AbstractLogger) :Promise<Bool>
	{
		...
		return Promise.promise(true)
			...
			.then(function(_) {
				...
				JobStatsTools.jobCopiedInputs(jobId, attempt);
				return true;
			});
	}
		...
		switch(job.image.type) {
			case Image:
				...
				return ensureDockerImage(docker, dockerImage, log, pull_options)
					...
					.then(function(_) {
						JobStatsTools.jobCopiedImage(jobId, attempt);
						return {error:error};
					});
			case Context:
				...
				if (container != null) {
					...
					return DockerJobTools.runDockerContainer(docker, opts, null, null, kill, log)
						.pipe(function(containerunResult) {
							...
							log = log.child({container:containerId});
							return JobStatsTools.setJobContainer(jobId, attempt, containerId)
								.pipe(function(_) {
									return JobStateTools.setWorkingStatus(jobId, attempt, JobWorkingStatus.ContainerRunning);
								})
								.then(function(_) {
				...
				if (container == null) {
					...
					DockerPromises.wait(container)
						.then(function(status :{StatusCode:Int}) {
							...
							JobStatsTools.jobContainerExited(jobId, attempt, exitCode, error);
							Node.clearTimeout(timeoutId);
							promise.resolve(true);
							promise = null;
						})
	...
	static function copyOutputsInternal(job :DockerBatchComputeJob, attempt :Int, dockerOpts :DockerConnectionOpts, fs :ServiceStorage, redis :RedisClient, log :AbstractLogger) :Promise<Array<String>>
	{
		...
		return DockerDataTools.lsVolume(outputsVolume)
			...
			.then(function(_) {
				JobStatsTools.jobCopiedOutputs(jobId, attempt);
				return outputFiles;
			});
	}

	static function copyLogsInternal(jobId :JobId, attempt :Int, docker :Docker, resultsStorageRemote :ServiceStorage, redis :RedisClient, log :AbstractLogger) :Promise<Bool>
	{
		return DockerJobTools.copyLogs(docker, jobId, resultsStorageRemote)
			.then(function(_) {
				JobStatsTools.jobCopiedLogs(jobId, attempt);
				return true;
			});
	}
}