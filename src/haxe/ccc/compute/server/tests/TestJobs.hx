...
class TestJobs extends ServerAPITestBase
{
	...
	{
		var testBlob = ServerTestTools.createTestJobAndExpectedResults('testJobDataAvailableAfterRemoval', 0);
		...
		return ClientJSTools.postJob(_serverHost, jobRequest)
			...
			.pipe(function(_) {
				traceCyan(JobCLICommand.Result);
			...
			.pipe(function(_) {
				traceCyan(JobCLICommand.ExitCode);
			...
			.pipe(function(_) {
				traceCyan(JobCLICommand.Definition);
			...
			.pipe(function(_) {
				traceCyan(JobCLICommand.JobStats);
