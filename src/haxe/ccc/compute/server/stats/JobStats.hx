...
import ccc.compute.server.stats.JobStats.JobStatsScripts.*;
...
abstract JobStats(RedisClient) from RedisClient
{
	...
	{
		return JobStatsScripts.evaluateLuaScript(this, SCRIPT_SET_JOB_REQUEST_RECIEVED, [jobId, time()]);
	}

	inline public function requestUploaded(jobId :JobId)
	{
		return JobStatsScripts.evaluateLuaScript(this, SCRIPT_SET_JOB_REQUEST_UPLOADED, [jobId, time()]);
	}

	inline public function jobFinished(jobId :JobId, ?error :String)
	{
		return JobStatsScripts.evaluateLuaScript(this, SCRIPT_SET_JOB_FINISHED, [jobId, time(), error]);
	}

	inline public function jobEnqueued(jobId :JobId)
	{
		return JobStatsScripts.evaluateLuaScript(this, SCRIPT_SET_JOB_ENQUEUED, [jobId, time()]);
	}

	inline public function jobDequeued(jobId :JobId)
	{
		return JobStatsScripts.evaluateLuaScript(this, SCRIPT_SET_JOB_DEQUEUED, [jobId, time()]);
	}

	inline public function jobCopiedInputs(jobId :JobId)
	{
		return JobStatsScripts.evaluateLuaScript(this, SCRIPT_SET_JOB_COPIED_INPUTS, [jobId, time()]);
	}

	inline public function jobCopiedImage(jobId :JobId)
	{
		return JobStatsScripts.evaluateLuaScript(this, SCRIPT_SET_JOB_COPIED_IMAGE, [jobId, time()]);
	}

	inline public function jobCopiedInputsAndImage(jobId :JobId)
	{
		return JobStatsScripts.evaluateLuaScript(this, SCRIPT_SET_JOB_COPIED_INPUTS_AND_IMAGE, [jobId, time()]);
	}

	inline public function jobContainerExited(jobId :JobId, exitCode :Int, ?error :String)
	{
		return JobStatsScripts.evaluateLuaScript(this, SCRIPT_SET_JOB_CONTAINER_EXITED, [jobId, time(), exitCode, error]);
	}

	inline public function jobCopiedOutputs(jobId :JobId)
	{
		return JobStatsScripts.evaluateLuaScript(this, SCRIPT_SET_JOB_COPIED_OUTPUTS, [jobId, time()]);
	}

	inline public function jobCopiedLogs(jobId :JobId)
	{
		return JobStatsScripts.evaluateLuaScript(this, SCRIPT_SET_JOB_COPIED_LOGS, [jobId, time()]);
	}
...
class JobStatsScripts
{
	...
	static var REDIS_KEY_HASH_JOB_STATS = '${PREFIX}v2';
...
';
	...
	static var scripts :Map<String, String> = [
		...
		',
		//jobs={{enqueued=0,dequeued=0,copiedInputs=0,copiedImage=0,copiedInputsAndImage=0,containerExited=0,copiedOutputs=0}
