...
class VagrantTools
{
	...
	public static function getMachineStatus(path :String) :Promise<StatusResult>
	{
	...
	static function __getMachineStatus(path :String) :Promise<StatusResult>
	{
		...
		machine.status(function(err, out) {
			...
			var status :StatusResult = Reflect.field(out, Reflect.fields(out)[0]);
