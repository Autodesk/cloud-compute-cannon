...
class ServiceTests
{
	...
	@rpc({
		alias:'monitor',
		doc:'Performs the most efficient full-stack test possible'
	})
	public function monitor() :Promise<Bool>
	{
...
';
		...
		return ccc.compute.client.js.ClientJSTools.postJob('http://localhost:9000${SERVER_RPC_URL}', request, forms)
			.pipe(function(jobResult :JobResultAbstract) {
				...
				var redis :js.npm.redis.RedisClient = _injector.getValue(js.npm.redis.RedisClient);
				return JobStatsTools.getPretty(jobResult.jobId);
			});
	}
